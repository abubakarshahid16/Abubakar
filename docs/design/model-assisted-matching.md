...
Status: DESIGN, not implemented. Written by Cowork 2026-09-19 against
`comparison.py` as it stood mid-containment task. Line numbers are approximate.
...
- fact is not already paired to another requirement in this run
- (requirement_id, fact_id) is not in `review_pair_rejections` (§7)
...
## 7. Engineer governance, two schema gaps that must close first

1. `review_findings` has no `confirmed_by / confirmed_at`, and
   `run_comparison(replace=True)` deletes every finding for the run
   unconditionally (~L622-627). Engineer dispositions are wiped on re-run.
   Fix: add the two columns; replace deletes only `WHERE confirmed_by IS NULL`,
   same as requirements and facts.
2. No memory of rejected pairings. Fix: table
   `review_pair_rejections(requirement_id, fact_id, rejected_by, rejected_at, reason)`.
   The pre-filter in §2 excludes rejected pairs, so a re-run never re-proposes
   a pairing an engineer already refused. Also record engineer-confirmed
   pairings (`METHOD_HUMAN`) so they are reused deterministically next run.

Both are prerequisites. A model tier without them produces findings an
engineer cannot durably accept or refuse.
