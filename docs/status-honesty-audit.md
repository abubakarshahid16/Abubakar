...
| 49 | Issue #175 (`datasheets._pairs_from_vision_fallback` docstring) and B54 (`reasoning_provider.py`): `OllamaProvider` is "exactly one provider that can actually make a model call today" | **It could not make one.** `model_transport.post_json` takes a keyword-only `timeout` with no default, and `OllamaProvider.reason` never passed it, so every real call raised `TypeError` and surfaced as `ProviderRefused`. Every provider test monkeypatched `post_json` with a fake taking `**kwargs`, so the missing argument was invisible; the seam had never carried a real request. Found by issue #180 on the first real vision call through the seam. **FIXED** (`Packet.timeout_s`, passed through), and pinned by a test whose fake has the real signature. Numbered 49 because entry 48 (the LanceDB retraction) lives on its own branch. **The rule: a fake of a transport has the transport's signature, or it tests a function that does not exist.** |
