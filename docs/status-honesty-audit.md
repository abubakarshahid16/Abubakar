...
| 17 | README: `python -m pytest -q` — **"887 tests, ~5 minutes"** | **1,174 passed / 3 skipped / 17 xfailed / 1 deselected** in **4m57s**, measured by CI at cd72bac. A local run of an earlier tree (5baf18e, four tests fewer) gave 1,170 in 5m44s with `--ignore` for the two parked test files. The count was understated by 287. The "~5 minutes" was close but low; a contended run with the dev server and Ollama up took over 11 minutes. Both figures were written once and never re-run |
