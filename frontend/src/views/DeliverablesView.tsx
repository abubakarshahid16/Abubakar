...
import { EmptyState, InlineErrorState, StatusBadge } from "../components/states";
  ...
  return (
    <main id="deliverables" className="mx-auto w-full max-w-6xl px-4 py-6">
      ...
      <section aria-label="Expected deliverables" className="mt-5 overflow-hidden rounded-[var(--radius-md)] border border-ink-600 bg-ink-850">
        ...
        {expected.length === 0 ? <EmptyState title="No expectations are configured or inferred yet." hint="Link a requirement document or add an expectation manually." /> : <div className="divide-y divide-ink-700/70">{expected.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3"><div><p className="text-sm text-slateish-200">{item.title}</p><p className="text-xs text-slateish-500">WBS {item.wbs_code} · {item.deliverable_type} · {item.origin}</p></div><span className={`rounded-full px-2 py-1 text-xs ${item.state === "missing" ? "bg-warn-500/15 text-warn-500" : "bg-signal-500/15 text-signal-300"}`}>{item.state}</span></div>)}</div>}
      </section>
