...
export function DeliverablesView() {
  ...
  return (
    <main id="deliverables" className="mx-auto w-full max-w-6xl px-4 py-6">
      ...
      {error !== null && <p role="alert" className="mt-4 rounded-[var(--radius-xs)] border border-danger-500/40 bg-danger-500/10 px-3 py-2 text-sm text-danger-500">{error}</p>}
      ...
      <section aria-label="Expected deliverables" className="mt-5 overflow-hidden rounded-[var(--radius-md)] border border-ink-600 bg-ink-850">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-ink-700 px-4 py-3">
          ...
          <span className="text-xs uppercase tracking-wide text-signal-400">{expected.filter((item) => item.state === "missing").length} missing</span>
        </div>
