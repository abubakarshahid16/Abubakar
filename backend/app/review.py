        ...
        conn.execute(
            """INSERT INTO review_findings
               (id, document_id, baseline_document_id, category, severity,
                requirement, finding, required_action, citation_ids,
                owner_user_id, due_date, status, approval_status,
            ...
            (
                ...
                payload.get("approval_status", "pending"), payload.get("escalation_level", 0),
                created_by, now, now,
            ),
        )
...
def update(finding_id: str, changes: dict) -> dict | None:
    ...
    allowed = {"owner_user_id", "due_date", "status", "approval_status",
               "escalation_level", "required_action", "severity"}
    sets: list[str] = []
    args: list[object] = []
